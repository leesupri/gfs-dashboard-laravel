    ...
    <form method="GET" action="{{ route('stock.counts.index') }}" class="flex flex-wrap gap-2 items-end">
      ...
      <div class="relative">
        <input type="text" name="search" value="{{ $filters['search'] ?? '' }}"
          placeholder="Search..." class="rounded-lg border pl-8 pr-3 py-1.5 text-xs focus:outline-none w-44"
          style="border-color:var(--card-border);color:var(--text-primary);background:var(--card-bg);">
    ...
    @if($counts->isEmpty())
      <div class="flex flex-col items-center gap-3 py-16 text-center">
        ...
        <div>
          ...
          <p class="mt-1 text-xs" style="color:var(--text-muted)">Adjust your filters or wait for mobile staff to submit counts.</p>
        </div>
      </div>
    @else
          ...
          <tbody>
            @foreach($counts as $count)
              <tr class="transition-colors hover:bg-gray-50" style="border-bottom:1px solid var(--card-border);">
                <td class="px-4 py-3 text-xs font-medium" style="color:var(--text-primary);">{{ $count->count_date->format('d M Y') }}</td>
                <td class="px-4 py-3 text-xs" style="color:var(--text-secondary);">{{ $count->warehouse_name }}</td>
                <td class="px-4 py-3 text-xs capitalize" style="color:var(--text-secondary);">{{ $count->count_type }}</td>
                <td class="px-4 py-3 text-xs" style="color:var(--text-secondary);">{{ $count->lines_count }}</td>
                <td class="px-4 py-3">
                ...
                <td class="px-4 py-3 text-xs" style="color:var(--text-secondary);">{{ $count->creator?->name ?? '—' }}</td>
                <td class="px-4 py-3 text-xs" style="color:var(--text-muted);">{{ $count->submitted_at?->format('d M Y, H:i') ?? '—' }}</td>
                <td class="px-4 py-3">
      ...
      @if($counts->hasPages())
        <div class="border-t px-4 py-3" style="border-color:var(--card-border);">{{ $counts->links() }}</div>
      @endif
    @endif
  </div>
