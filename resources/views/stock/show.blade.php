    ...
    <div x-show="showApproveModal" x-cloak>
      ...
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="closeApprove()">
        <div class="w-full max-w-md rounded-2xl p-6"
          ...
          <p class="mb-5 text-sm" style="color:var(--text-secondary)">This will mark the stock count as approved and feed it into audit comparisons. This action cannot be undone.</p>
        ...
        <div class="w-full max-w-md rounded-2xl p-6"
          ...
          <form action="{{ route('stock.counts.reject', $count->id) }}" method="POST" class="space-y-4">
            ...
            <div class="flex justify-end gap-3">
              ...
              <button type="submit"
                      class="rounded-xl px-5 py-2 text-sm font-semibold text-white transition hover:-translate-y-0.5 active:scale-95"
                      style="background:#dc2626; box-shadow:0 4px 16px rgba(220,38,38,0.25)">
                Reject
              </button>
            </div>
          </form>
