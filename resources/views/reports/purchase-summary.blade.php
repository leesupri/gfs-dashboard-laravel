...
<div x-data="{ filtersOpen: {{ request()->except('page') ? 'true' : 'false' }} }" class="space-y-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500">Purchase summary by category and item.</p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div class="rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-sm">
            ...
            <div class="font-semibold text-gray-900">{{ number_format($rows->total()) }}</div>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($rows as $row)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 text-right">Rp {{ number_format((float)$row->totalCost, 2, '.', ',') }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="px-4 py-6 text-sm text-gray-500">No data found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
