...
<div x-data="{ filtersOpen: {{ (request('q') || request('start') || request('end') || request('dept')) ? 'true' : 'false' }} }" class="space-y-6">
  ...
  <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div class="flex items-center gap-2">
      <a
  href="{{ route('itemSales.export', request()->query()) }}"
...
>
    ...
    <div>
      ...
      >
        ...
        @foreach($departments as $dept)
          <option value="{{ $dept }}" {{ ($filters['dept'] ?? '') === $dept ? 'selected' : '' }}>
            {{ $dept }}
          </option>
        @endforeach
      </select>
  ...
  <div class="mt-4 flex flex-wrap gap-2 border-t pt-4">
    ...
    <a href="{{ route('itemSales.index', array_merge(request()->except(['start','end']), [
      'start' => now()->toDateString(),
      'end' => now()->toDateString()
    ])) }}"
      class="rounded-lg border px-3 py-1.5 text-xs hover:bg-gray-50">
      Today
    </a>
...
@forelse($byDept as $dept => $cats)
  <tr>
  <td colspan="11" class="h-3 bg-gray-50"></td>
</tr>
  {{-- Department header --}}
  ...
  @if($hasService)
  <td class="px-3 py-2 text-right">{{ number_format($service, 2, ',', '.') }}</td>
@endif
