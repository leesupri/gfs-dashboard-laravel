...
class SalesConsumptionDetailInvoiceController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Use plain arrays for grouping to avoid Collection memory overhead
        $byInvoice  = [];
        $grandTotal = 0.0;
        foreach ($rows as $row) {
            $key = $row->invoice_id;
            $byInvoice[$key][] = $row;
            $grandTotal += (float) $row->totalCost;
        }
    ...
    public function export(Request $request)
    {
        ...
        $headers = ['Invoice', 'Date', 'Result', 'Item', 'Qty', 'UOM', 'Warehouse', 'Total Cost'];
        ...
        return ExcelExport::download(
            "consumption_detail_invoice_{$start}_to_{$end}.xlsx",
            'Sales Consumption Detail per Invoice',
            $headers,
            $dataRows
        );
    }
